...
থাকায় আরও বড় দেখায়। ওই একই নিয়মে ২০২২-২৫ = **-$10,810** (০.০১ লট), ২০২৬ জানু-সেপ = **-$1,502**;
...
### 🟥 লাল বক্স ফিল্টার ("Trande Hunter — Permanent Stalling Red Box") — যাচাই ও উন্নতি

আপনার পেস্ট করা স্ক্রিপ্ট হুবহু পোর্ট করা হয়েছে (প্রতিটা confirmed pivot-এ ব্যান্ড আপডেট,
ব্যান্ডে ঢুকলেই বক্স খোলে, close ভিতরে থাকা পর্যন্ত সক্রিয়, বাইরে গেলে চিরতরে লক) এবং
`verify_redbox_fidelity.py` লিটারেল Pine অনুবাদের সাথে **বিট-বাই-বিট** মিলিয়ে দেখে
(2022: ৩,৫৪,৬২৮ বারে ০ পার্থক্য)।

**১) দাবির যাচাই (Sep ২০২৬, ৭ দিন — ০.০১ লট, $0.20/ট্রেড):**

| | ট্রেড | win% | net |
|---|---|---|---|
| বক্স ছাড়া | 255 | 53.3% | +$72.60 |
| বক্স সহ (আপনার সেটিংস) | **111** | **63.1%** | **+$123.94** (10 oz: +$1,239.43) |

**২) আপনার "৩১%" বেসলাইন ঠিক, "৬৮%" নয়** — ৪ বছরের **৯৫১টা ৭-দিনের উইন্ডো** স্ক্যান:

| | unfiltered | বক্স (skip) | বক্স (defer) |
|---|---|---|---|
| median win% | **31.1%** | 32.5% | **37.6%** |
| win rate বাড়ে | — | **৯৭% উইন্ডোতে**, median +6.5 পয়েন্ট | |
| সর্বোচ্চ win% | — | 63.0% | 62.8% |
| ≥68% উইন্ডো | — | **০টা** | **০টা** |

সেরা উদাহরণ: 2025-08-17 — ২৩৪ ট্রেড ৩২.৫% −$66 → বক্সে ১৬৩ ট্রেড **৫০.৩%**।

**৩) বক্সের দাম:** ৪ বছরে **৫২,১১১ → ১৩,৪৭৮ ট্রেড (৭৪% উধাও)**। ব্লক করা ৩৫,৫৩১
ট্রেডের win ৩০.০% (মোট −$8,622), রাখা ট্রেড ৩৭.৯% (−$2,527) — ফিল্টার খারাপ ট্রেড
সত্যিই চেনে, কিন্তু সংখ্যাটা মুছে দেয়।

**৪) উন্নতি — DEFERRED ENTRY (ট্রেড না কমিয়ে লস আটকানো):** সিগন্যাল বক্সের ভিতরে পড়লে
বাদ না দিয়ে `pending` করে রাখা হয়; বক্স শেষ হওয়া মাত্র ওই দিকেই এন্ট্রি।

| | 4y ট্রেড (kept) | 4y win% | 4y net | Sep26 ট্রেড | win% | net |
|---|---|---|---|---|---|---|
| বক্স ছাড়া | 52,111 (100%) | 32.5% | −$11,149 | 255 | 53.3% | +$72.60 |
| পেস্ট করা ব্লক | 13,478 (26%) | 35.6% | **−$3,182** | 111 | 63.1% | +$123.94 |
| **DEFER (box_end)** | **35,805 (69%)** | **39.5%** | −$7,793 | **207 (81%)** | **65.2%** | **+$171.00** |

ফ্যামিলি ও রোবাস্টনেস: ব্যান্ড $3–$12 × fail 2–4 — সব সেটিংসেই ৪ বছর win **৩৮.৫–৩৯.৬%**,
ট্রেড **৬৪–৭৪%** টিকে, Sep-26 ১২টির ১০টিতে বেসলাইনের চেয়ে ভালো। ATR-স্কেল ব্যান্ড কাজ
করে না (৯৩% ট্রেড টিকে যায়, win বদলায় না); একদিক ব্লকও খারাপ।

**বিনিময়টা স্পষ্ট:** ব্লক = সবচেয়ে কম লস (−$3,182) কিন্তু ট্রেড ২৬%; DEFER = ট্রেড ৬৯%,
win rate সবচেয়ে বেশি (৩৯.৫%) কিন্তু net −$7,793। বাদ পড়া সিগন্যাল খারাপই — ফিরিয়ে আনলে
তারা আবার লস করে। চার বছরে কোনো ভ্যারিয়েন্ট নেট পজিটিভ নয়: ১-মিনিট EMA ক্রসে ৪ বছরে
$0.20/oz স্প্রেডই ~$10.4k — ফিল্টার লস কমায়, এজ বানায় না।

📄 `results/redbox_report.md` · `results/redbox.png` · `results/redbox_windows.png`
🧩 `pine/redbox_deferred.pine` (TradingView-তে পেস্ট করার মতো DEFERRED প্যাচ)
🔍 `verify_redbox_fidelity.py` (পোর্ট বনাম লিটারেল Pine: বিট-বাই-বিট অডিট)
...
├── run_redbox.py             # 🟥 লাল বক্স ফিল্টার: পোর্ট + দাবির যাচাই + DEFER উন্নতি
├── verify_redbox_fidelity.py # 🟥 পোর্ট বনাম লিটারেল Pine অনুবাদ (বিট-বাই-বিট অডিট)
├── pine/redbox_deferred.pine # 🟥 DEFERRED-এন্ট্রি Pine প্যাচ (পেস্ট করার জন্য)
...
python3 run_redbox.py           # লাল বক্স ফিল্টার: যাচাই, ফ্যামিলি, DEFER উন্নতি
python3 verify_redbox_fidelity.py  # পোর্টের ফিডেলিটি অডিট (Pine বনাম Python)
```
